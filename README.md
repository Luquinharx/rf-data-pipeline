...
1. **Instale as dependências:**
   Certifique-se de ter o Python instalado e as bibliotecas necessárias (pandas, requests, tqdm, etc.):
   ```bash
   pip install pandas requests tqdm python-dateutil numpy
   ```

2. **Execute o Crawler (Opcional se já tiver os dados):**
   ```bash
   python crawler_receita.py
   ```

3. **Processe a Base da Receita:**
   Execute a formatação apontando para o arquivo baixado (o crawler geralmente chama esses passos automaticamente ou você deve rodar manualmente se tiver o arquivo bruto).
   ```bash
   python formatar_csv.py CAMINHO_DO_ARQUIVO_BRUTO
   ```

4. **Execute os Cruzamentos:**
   Verifique se os arquivos de entrada (`111_CUC_ATIVO.csv`, `Matera1.csv`) estão na pasta `Temporario/`.
